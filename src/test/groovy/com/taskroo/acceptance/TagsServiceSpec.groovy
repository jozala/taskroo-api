...
class TagsServiceSpec extends AcceptanceTestBase {
    ...
    def cleanup() {
        ...
        securityTokensCollection.remove(QueryBuilder.start('user_id').in(['owner1Login', TEST_USER_ID]).get())
    }
    ...
    def "should return all tags of the specified user in the JSON format and status code 200"() {
        ...
        prepareTestData()
        def securityTokenId = createSecurityTokenWithUser('owner1Login')
        when: "client sends POST request to /task to create a new task"
        HttpResponseDecorator response = client.get(path: 'tags', headers: ['Authorization': generateAuthorizationHeader(securityTokenId)])
    ...
    def "should create new tag when client sends POST request with tag"() {
        ...
        def response = client.post(path: 'tags', headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: tag, requestContentType: ContentType.JSON)
    ...
    def "should return 200 and tag data when trying to create tag with existing name"() {
        ...
        def previousResponse = client.post(path: 'tags', headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: tag, requestContentType: ContentType.JSON)
    ...
    def "should remove tag from DB when client sends DELETE request with tag ID"() {
        ...
        def securityTokenId = createSecurityTokenWithUser(TEST_USER_ID)
        def tag = '{"name": "abc", "color": "gray", "visibleInWorkView": true}'
        def existingTagResponse = client.post(path: 'tags', headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: tag, requestContentType: ContentType.JSON)
        when: "client sends DELETE request with tag ID"
        def response = client.delete(path: "tags/${existingTagResponse.data.id}", headers: ['Authorization': generateAuthorizationHeader(securityTokenId)])
    ...
    def "should return 404 (not found) when trying to remove non-existing tag"() {
        ...
        def securityTokenId = createSecurityTokenWithUser(TEST_USER_ID)
        def response = client.delete(path: "tags/${ObjectId.get().toString()}", headers: ['Authorization': generateAuthorizationHeader(securityTokenId)])
    ...
    def "should remove tag from all tasks when removing tag"() {
        ...
        def existingTaskResponse = client.post(path: 'tasks', headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: '{"title": "taskTitle1", "tags":[{"name":"abc"}]}', requestContentType: ContentType.JSON)
        when: "client sends DELETE request to remove tag 'abc'"
        client.delete(path: "tags/${existingTagResponse.data.id}", headers: ['Authorization': generateAuthorizationHeader(securityTokenId)])
    ...
    def "should update tag fields and return updated tag when update request received"() {
        ...
        def tagAfterUpdateResponse = client.put(path: "tags/$existingTagResponse.data.id", headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: updatedTag, requestContentType: ContentType.JSON)
    ...
    def "should return 404 when trying to update non-existing tag"() {
        ...
        def response = client.put(path: "tags/${ObjectId.get().toString()}", headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: someTag, requestContentType: ContentType.JSON)
    ...
    def "should return 400 (bad request) when trying to update task to have same name as other already existing tag"() {
        ...
        client.post(path: 'tags', headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: tagAbc, requestContentType: ContentType.JSON)
        def cdeResponse = client.post(path: 'tags', headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: tagCde, requestContentType: ContentType.JSON)
        when: "client sends request to update 'cde' tag's name to 'abc'"
        def response = client.put(path: "tags/$cdeResponse.data.id", headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], body: tagAbc, requestContentType: ContentType.JSON)
    ...
    def "should return 400 (bad request) when client sends PUT request with tag id and no body"() {
        ...
        def tagAfterUpdateResponse = client.put(path: "tags/$existingTagResponse.data.id", headers: ['Authorization': generateAuthorizationHeader(securityTokenId)], requestContentType: ContentType.JSON)
