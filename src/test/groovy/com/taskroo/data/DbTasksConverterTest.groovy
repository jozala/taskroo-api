...
class DbTasksConverterTest extends Specification {
    ...
    List<DBObject> dbTasks = [
            ...
            new BasicDBObject([_id: '1', title: 'title 1', finished: false, created_date: new Date(), owner_id: 'ownerId', path: []]),
            new BasicDBObject([_id: '11', title: 'title 11', finished: false, created_date: new Date(), owner_id: 'ownerId', path: ['10']]),
            new BasicDBObject([_id: '9', title: 'title 9', finished: false, created_date: new Date(), owner_id: 'ownerId', path: []]),
            new BasicDBObject([_id: '10', title: 'title 10', finished: false, created_date: new Date(), owner_id: 'ownerId', path: []]),
    ...
    def "should put tasks with no elements in path as top level tasks"() {
        when:
    ...
    def "should add lower level subtasks to proper tasks"() {
        ...
        tasks.find { it.id == '1' }.subtasks.find { it.id == '5' }.subtasks.find { it.id == '6' }.subtasks.collect {
            it.id
        } as Set == ['7', '8'] as Set
